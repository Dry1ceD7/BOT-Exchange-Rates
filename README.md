...
- **SQLite Persistence & Resilience:** (v1.3.9) Leverages a local persistence layer to cache rates, ensuring 100% data density even during API throttling (429 errors).
...
- **Accounting Automation:** (v1.3.9) Includes support for `bot_acc_filler.py` to automatically fill customs entry/accounting files with the correct historical rates.
- **Precision Financial Compliance:** Supports high-precision calculations (4+ decimal places) and Thai fiscal holiday integration.
...
| `bot_generator.py` | Raw CSV generator and main data engine |
| `bot_core.py` | (v1.3.9) Shared logic engine and SQLite persistence manager |
| `bot_acc_filler.py` | (v1.3.9) Accounting automation tool for customs entries |
| `config.json` | Centralized API, currency, and holiday configuration |
| `.env` | API token configuration (Excluded from git) |
