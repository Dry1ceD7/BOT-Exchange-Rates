...
core/prescan.py
---------------------------------------------------------------------------
BOT Exchange Rate Processor (v4.0) - Smart Date Pre-Scanner
---------------------------------------------------------------------------
Pre-scans queued .xlsx files to detect the oldest date in the
source column. Uses openpyxl exclusively (.xls no longer supported).
"""
...
def prescan_oldest_date(
    ...
    for fp in filepaths:
        ...
        # Skip legacy .xls files
        if fp.lower().endswith(".xls") and not fp.lower().endswith(".xlsx"):
            logger.warning("Skipping unsupported .xls file: %s", os.path.basename(fp))
            continue
